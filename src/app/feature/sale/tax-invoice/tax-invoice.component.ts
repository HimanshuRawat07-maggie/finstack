...
export class TaxInvoiceComponent {
  ...
  loadTableData() {
    ...
    this.unpaid = 0;
    this.total = 0;
    this.saleService.getAllTaxInvoice(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected tax invoice?',
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.saleService.deleteTaxInvoice(id!).subscribe(response => {
          this.loadTableData();
  ...
  hasPermission(permissionValue: string) {
    return BusinessHelpers.hasPermission(this.userPermissions, ModuleConstants.TaxInvoice, permissionValue);
  }
