...
@Component({
  selector: 'app-create-export-invoice',
  templateUrl: './create-export-invoice.component.html',
  styleUrls: ['./create-export-invoice.component.scss']
})
export class CreateExportInvoiceComponent {
 public constants = Constants;
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(parameterMap => {
      ...
      if (id != null) {
        const sub = this.saleService.getExportInvoiceById(parseInt(id)).subscribe(res => {
        ...
        this.saleService.getLatestOrderNumber(this.constants.Export_Invoice).subscribe(res => {
          this.order.orderNumber = res.data;
        });

        const suff = this.companyService.getCompanySettingByKey('exportinvoice.suffix').subscribe(res => {
          this.suffix = res.data;
          suff.unsubscribe();
        });
        const pref = this.companyService.getCompanySettingByKey('exportinvoice.prefix').subscribe(res => {
  ...
  saveOrder(orderData: SalePurchase) {
    ...
    if (order.id && order.id > 0) {
      const updateSub = this.saleService.updateExportInvoice(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/sale/export-invoice');
      ...
      const saveSub = this.saleService.saveExportInvoice(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/sale/export-invoice');
