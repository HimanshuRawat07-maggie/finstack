...
export class ServiceInvoiceComponent {
  ...
  loadTableData() {
    ...
    this.unpaid = 0;
    this.total = 0;
    this.saleService.getAllServiceInvoice(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected service invoice?',
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.saleService.deleteServiceInvoice(id!).subscribe(response => {
          this.loadTableData();
