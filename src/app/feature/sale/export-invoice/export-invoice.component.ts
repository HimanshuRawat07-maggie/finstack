...
export class ExportInvoiceComponent {
 fromDate: string = '';
  ...
  loadTableData() {
    ...
    this.unpaid = 0;
    this.total = 0;
    this.saleService.getAllExportInvoice(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected export invoice?',
        title: 'Delete Export Invoice'
      },
    });
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.saleService.deleteExportInvoice(id!).subscribe(response => {
          this.loadTableData();
  ...
  hasPermission(permissionValue: string) {
    return BusinessHelpers.hasPermission(this.userPermissions, ModuleConstants.ServiceInvoice, permissionValue);
  }
