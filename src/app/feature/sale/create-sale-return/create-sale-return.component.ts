...
@Component({
  ...
  styleUrls: ['./create-sale-return.component.scss']
})
export class CreateSaleReturnComponent implements OnInit {
  ...
  constructor(private saleService: SaleService, private router: Router, private dialog: MatDialog, private route: ActivatedRoute,
    ...
    this.route.paramMap.subscribe(parameterMap => {
      ...
      if (type == 'edit' && id != null) {
        ...
        const sub = this.saleService.getSaleInvoiceById(parseInt(id)).subscribe(res => {
          ...
          this.saleService.getLatestOrderNumber(this.constants.SaleReturn).subscribe(res => {
            this.order.orderNumber = res.data;
          });
          sub.unsubscribe();
        });

        const suff = this.companyService.getCompanySettingByKey('creditnote.suffix').subscribe(res => {
          this.suffix = res.data;
          suff.unsubscribe();
        });
        const pref = this.companyService.getCompanySettingByKey('creditnote.prefix').subscribe(res => {
          this.prefix = res.data;
          pref.unsubscribe();
        });

      }  else if (type == 'tax' && id != null) {
        const sub = this.saleService.getTaxInvoiceById(parseInt(id)).subscribe(res => {
          this.order = BusinessHelpers.MapFromApiModel(res.data);
          this.order.id = -1;
          this.order.orderDate = this.datePipe.transform(new Date(), 'yyyy-MM-dd');
          this.order.referenceId = parseInt(id);
  ...
  saveOrder(orderData: SalePurchase) {
    ...
    if (order.id && order.id > 0) {
      const updateSub = this.saleService.updateSalesReturn(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/sale/return');
      ...
      const saveSub = this.saleService.saveSalesReturn(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/sale/return');
