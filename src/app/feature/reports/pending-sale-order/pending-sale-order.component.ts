...
export class PendingSaleOrderComponent {
  fromDate: string = '';
  toDate: string = '';
  ...
  loadTableData() {
    const sub = this.reportService.getAllPendingSaleOrderReport(this.partyId, this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
      sub.unsubscribe();
    });
  }
  ...
  download() {
    ...
    let url = ApiUrl.exportPendingSaleOrderReport
    ...
    this.reportService.downloadReport(url).subscribe(res => {
      ...
      anchorElement.download = 'Pending Sale Order Report.xlsx';
      anchorElement.click();
  ...
  downloadAsPdf(paperOrientation: string) {
    this.reportService.processing(true);
    let url = ApiUrl.exportPendingSaleOrderReportPdf;
    if (this.fromDate && this.toDate) {
    ...
    this.reportService.downloadReport(url).subscribe(res => {
      ...
      anchorElement.href = objectURL;
      anchorElement.download = 'Pending Sale Order Report.pdf';
      anchorElement.click();
      document.body.removeChild(anchorElement);
      this.reportService.processing(false);
    });
