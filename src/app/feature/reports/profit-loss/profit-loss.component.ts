...
export class ProfitLossComponent {
  ...
  refreshReport() {
    ...
    const sub = this.reportService
      .profitAndLoss(this.fromDate, this.toDate)
      .subscribe((res) => {
        ...
        this.indirectExpenses = this.reportData.filter(
          (x) =>
            x.title === 'Indirect Expenses' || x.title === 'Indirect Incomes'
        );
        this.reportData = this.reportData.filter(
          (x) => x.title != 'Indirect Expenses' && x.title != 'Indirect Incomes'
        );
        ...
        let netDebit =
          ...
          this.indirectExpenses
            .filter((x) => x.depth === 0)
            .reduce((a, v) => (a += Math.abs(v.debitAmount)), 0);
        let netCredit =
          grossCredit +
          this.indirectExpenses
            .filter((x) => x.depth === 0)
            .reduce((a, v) => (a += Math.abs(v.creditAmount)), 0);
        ...
        let directExpIdx = this.reportDebitSideData.findIndex(
          (x) => x.title === 'Direct Expenses'
        );
        this.reportDebitSideData.splice(
          directExpIdx + 1,
          this.reportDebitSideData.length - directExpIdx
        );
        this.reportCreditSideData.splice(0, directExpIdx + 1);
        this.reportDebitSideExpenseData = [
          ...this.indirectExpenses.filter(
            (x) => x.title === 'Indirect Expenses'
          ),
        ];
        this.reportCreditSideIncomeData = [
          ...this.indirectExpenses.filter(
            (x) => x.title === 'Indirect Incomes'
          ),
        ];
  ...
  downloadAsPdf(paperOrientation: string) {
    ...
    let url = ApiUrl.exportProfitLossReportPdf;
    if (this.fromDate && this.toDate) {
      url =
        url +
        `${this.fromDate}&endDate=${this.toDate}&paperOrientation=${paperOrientation}`;
    }
    this.reportService.downloadReport(url).subscribe((res) => {
      ...
      anchorElement.download = 'Profit & Loss Report.pdf';
      anchorElement.click();
  ...
  removePurchaseSaleAccount(
    data: Array<TrialBalanceItem>
  ): Array<TrialBalanceItem> {
    return data.filter(
      (x) => x.title != 'Purchase Account' && x.title != 'Sales Account'
    );
  }
  ...
  addTotalFromChildren(data: Array<TrialBalanceItem>) {
    data.forEach((node) => {
      if (node?.children?.length >= 0) {
        ...
        node.debitAmount += node.children.reduce(
          (a, v) => (a += Math.abs(v.debitAmount)),
          0
        );
        node.creditAmount += node.children.reduce(
          (a, v) => (a += Math.abs(v.creditAmount)),
          0
        );
      }
    });
  ...
  downloadPdf() {
    ...
    this.companyService.getCompanyDetailsById().subscribe((res) => {
      ...
      const reportName = 'Profit & Loss'; // Name of the report
    ...
    download() {
    this.reportService.processing(true);
    let url = ApiUrl.exportProfitAndLossReport
    if (this.fromDate && this.toDate) {
    ...
    this.reportService.downloadReport(url).subscribe(res => {
      ...
      anchorElement.href = objectURL;
      anchorElement.download = 'Profit & Loss Report.xlsx';
      anchorElement.click();
      document.body.removeChild(anchorElement);
      this.reportService.processing(false);
    });
