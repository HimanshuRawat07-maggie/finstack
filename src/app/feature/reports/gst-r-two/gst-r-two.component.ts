...
@Component({
  selector: 'app-gst-r-two',
  templateUrl: './gst-r-two.component.html',
  styleUrls: ['./gst-r-two.component.scss']
})
export class GstRTwoComponent {
 fromDate: string = '';
  toDate: string = '';
  data: Array<GSTR1> = []
  ...
  loadTableData() {
    const sub = this.reportService.getGstR2Report(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
      sub.unsubscribe();
    });
  }
