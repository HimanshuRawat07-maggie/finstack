...
@Component({
  ...
  styleUrls: ['./purchase-bills.component.scss'],
  providers: [DatePipe]
})
export class PurchaseBillsComponent {
  ...
  loadTableData() {
    ...
    this.unpaid = 0;
    this.total = 0;
    const sub = this.purchaseService.getAllPurchaseBills(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected purchase bill?',
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.purchaseService.deletePurchaseBill(id!).subscribe(response => {
          this.loadTableData();
  ...
  hasPermission(permissionValue: string, module?: string) {
    if (module == 'Return') {
      return BusinessHelpers.hasPermission(this.userPermissions, ModuleConstants.DebitNote, permissionValue);
    }
    else {
