...
@Component({
  selector: 'app-challan-in-dashboard',
  templateUrl: './challan-in-dashboard.component.html',
  styleUrls: ['./challan-in-dashboard.component.scss']
})
export class ChallanInDashboardComponent {
 data: Array<DebitNote> = [];
  ...
  loadTableData() {
    const sub = this.purchaseService.getAllChallanIn(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected challan-in?',
        title: 'Delete Challan-In'
      },
    });
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.purchaseService.deleteChallanIn(id!).subscribe(response => {
          this.loadTableData();
