...
export class PurchaseReturnComponent implements OnInit {
  ...
  loadTableData() {
    ...
    this.unpaid = 0;
    this.total = 0;
    const sub = this.purchaseService.getAllDebitNote(this.fromDate, this.toDate).subscribe(res => {
      this.data = res.data;
  ...
  openDeleteDialog(idx: number) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      autoFocus: false,
      data: {
        message: 'Are you sure you want to delete the selected purchase return?',
    ...
    dialogRef.componentInstance.confirmed.subscribe(res => {
      if (res) {
        ...
        this.purchaseService.deletePurchaseReturn(id!).subscribe(response => {
          this.loadTableData();
