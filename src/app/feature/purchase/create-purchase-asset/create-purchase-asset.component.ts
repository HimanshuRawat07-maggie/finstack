...
@Component({
  selector: 'app-create-purchase-asset',
  templateUrl: './create-purchase-asset.component.html',
  styleUrls: ['./create-purchase-asset.component.scss']
})
export class CreatePurchaseAssetComponent {
 public constants = Constants;
  public order: SalePurchase;
  public prefix: string;
  public suffix: string;

  constructor(private purchaseService: PurchaseService, private dialog: MatDialog, private router: Router, private saleService: SaleService, private route: ActivatedRoute,
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(parameterMap => {
      ...
      if (id != null) {
        const sub = this.purchaseService.getPurchaseAssetById(parseInt(id)).subscribe(res => {
        ...
        this.saleService.getLatestOrderNumber(this.constants.PurchaseAssets).subscribe(res => {
          this.order.orderNumber = res.data;
        });

        const suff = this.companyService.getCompanySettingByKey('purchaseassets.suffix').subscribe(res => {
          this.suffix = res.data;
          suff.unsubscribe();
        });
        const pref = this.companyService.getCompanySettingByKey('purchaseassets.prefix').subscribe(res => {
  ...
  saveOrder(orderData: SalePurchase) {
    ...
    if (order.id && order.id > 0) {
      const updateSub = this.purchaseService.updatePurchaseAssets(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/purchase/assets');
      ...
      const saveSub = this.purchaseService.savePurchaseAsset(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/purchase/assets');
