...
export class CreatePurchaseComponent implements OnInit {
    ...
    this.route.paramMap.subscribe(parameterMap => {
      ...
      if (type == 'edit' && id != null) {
        ...
        const sub = this.saleService.getSaleInvoiceById(parseInt(id)).subscribe(res => {
          ...
          this.saleService.getLatestOrderNumber(this.constants.Purchase).subscribe(res => {
            this.order.orderNumber = res.data;
          });
          sub.unsubscribe();
        });

        const suff = this.companyService.getCompanySettingByKey('purchase.suffix').subscribe(res => {
          this.suffix = res.data;
          suff.unsubscribe();
        });
        const pref = this.companyService.getCompanySettingByKey('purchase.prefix').subscribe(res => {
  ...
  saveOrder(orderData: SalePurchase) {
    ...
    if (order.id && order.id > 0) {
      const updateSub = this.purchaseService.updatePurchaseBills(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/purchase/bills');
      ...
      const saveSub = this.purchaseService.savePurchaseBills(order).subscribe(res => {
        if (res.code === 200) {
          this.router.navigateByUrl('/app/purchase/bills');
