...
@Component({
  ...
  styleUrls: ['./add-payment-in.component.scss'],
})
export class AddPaymentInComponent implements OnInit {
  ...
  @ViewChild('paymentInFrom') paymentInFrom?: any;
  constructor(private partyService: PartyService, private expenseService: ExpenseService, private route: ActivatedRoute, private dialog: MatDialog, private datePipe: DatePipe, private saleService: SaleService, private toastr: ToastrService, private router: Router, private companyService: CompanyService, private bankService: BankService) { }
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(parameterMap => {
      ...
      if (id != null) {
        this.status = 'Edit'
        const get = this.saleService.getPaymentInById(parseInt(id)).subscribe(res => {
          this.data = res.data;
        ...
        const suff = this.companyService.getCompanySettingByKey('paymentin.suffix').subscribe(res => {
          this.suffix = res.data;
          this.data.orderSuffix = this.suffix;
          suff.unsubscribe();
        });
        const pref = this.companyService.getCompanySettingByKey('paymentin.prefix').subscribe(res => {
          this.prefix = res.data;
          this.data.orderPrefix = this.prefix;
          pref.unsubscribe();
        });
        const billSub = this.saleService.getLatestOrderNumber(Constants.PaymentIn).subscribe(res => {
  ...
  onSubmit() {
    ...
    this.paymentInFrom.control.markAllAsTouched();
    if (!this.paymentInFrom.form.valid)
      return;

    this.data.orderNumber = this.data.orderNumber?.toString();
    if (this.data.id == null) {
      const sub = this.saleService.savePaymentIn(this.data).subscribe(res => {
        if (res.code == 200) {
          this.router.navigateByUrl('/app/other-entries/payment-in');
      ...
      const sub = this.saleService.updatePaymentIn(this.data).subscribe(res => {
        if (res.code == 200) {
          this.router.navigateByUrl('/app/other-entries/payment-in');
  ...
  getTransactionById(check: boolean) {    
    ...
    if (this.selectedPartyLedger.id && this.selectedPartyLedger?.group === 'Party') {
      ...
      const sub = this.saleService.unPaidPaymentInByPartyId(this.selectedPartyLedger.id, partyOrLedgerId).subscribe(res => {
        if (res.code === 200) {
